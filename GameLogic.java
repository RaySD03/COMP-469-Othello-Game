...
import java.awt.Node;

import org.w3c.dom.Node;

public class GameLogic {
    
     public static char[][] validMoves(int color, int[][] board){ //returns list of valid moves as int array (I don't know a good way to make number pairs in Java yet)
       char[][] validList = new char[8][8];
        for (int ypos = 0; ypos < 8; ypos++){
            for (int xpos = 0; xpos < 8; xpos++){
                if (isValid(color, xpos, ypos, board)) { 
                   validList[ypos][xpos] = 't';
                }
            }
        }
